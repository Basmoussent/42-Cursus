...
int	check_cases(char **str, t_info map)
{
	...
	while (spawn.x < map.rows)
	{
		...
		while (spawn.y < map.cols)
		{
			if (str[spawn.x][spawn.y] == 'P' || str[spawn.x][spawn.y] == '1'
				|| str[spawn.x][spawn.y] == 'E'
				|| str[spawn.x][spawn.y] == '0'
				|| str[spawn.x][spawn.y] == 'K'
				|| str[spawn.x][spawn.y] == 'C')
				;
			else
				return (1);
			spawn.y++;
